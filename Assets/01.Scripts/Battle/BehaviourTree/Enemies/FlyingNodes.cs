...
public class FlyingPatrolAttackNode : Node
{
    ...
    public override void OnAnimatedEvent(bool isFire)
    {
        if (isFire)
        {
            ProjectileManager.Instance.CreateProjectile(controller.transform, controller.attack, attrs: new []{ new StraightAttr()}, degree: -90, index: 0);
            ProjectileManager.Instance.CreateProjectile(controller.transform, controller.attack, attrs: new []{ new StraightAttr()}, degree: -80, index: 0);
            ProjectileManager.Instance.CreateProjectile(controller.transform, controller.attack, attrs: new []{ new StraightAttr()}, degree: -100, index: 0);
