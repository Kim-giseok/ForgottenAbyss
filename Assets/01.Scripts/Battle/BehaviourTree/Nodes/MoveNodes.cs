...
public class PatrolMove : Node
{
    ...
    public override void Start()
    {
        ...
        if (!controller.Detect.isWalkable)
        {
            controller.Flip(direction != Vector2.right);
...
public class TracingNode : Node
{
    ...
    public override void Update()
    {
        // 플랫폼을 우선 체크하여 넘어갈 수 없는 상황이라면 점프(notify 형태로 관리하기)
...
public class DashNode : Node // 현재 방향이거나 타겟 방향
{
    ...
    public override void Update()
    {
        ...
        controller.Rigid.velocity = new Vector2(Mathf.Approximately(controller.transform.localEulerAngles.y, 180) ? -2 : 2, controller.Rigid.velocity.y);
    }
}
